...
fn main() -> Result<(), std::io::Error> {
 ...
 
    {
        ...
        while let Some(Ok(first)) = res.next(){
            ...
            let intersection : HashSet<char> = first_set.intersection(&second_set).map( |x| *x).collect();
            let intersection:  Vec<char> = intersection.intersection(&third_set).map( |x| *x).collect();
