    ...
    match map.get(&start.x) {
        Some(x) => match x.iter().find(|y| y >= &&start.y) {
            Some(y) => {
                ...
                let is_left_taken = map.get(&(v_left.x))
                .map(|z| z.contains(&v_left.y)).unwrap_or(false);
  
                if !is_left_taken { return fall(&v_left,map)}

                let v_right = Vec2{x:f.x+1,y:f.y};
                let is_right_taken = map.get(&(v_right.x))
                .map(|z| z.contains(&v_right.y)).unwrap_or(false);
                
                if !is_right_taken { return fall(&v_right,map)}

                return Some(Vec2{x:start.x, y:*y-1})
            },
            _ => None
        },
        _ => None
    ...
    match map.get(&start.x) {
        Some(x) => match x.iter().find(|y| y >= &&start.y) {
            Some(y) => {
                ...
                if !is_left_taken { return fall2(&v_left,y_max, map)}

                let v_right = Vec2{x:f.x+1,y:f.y};
                let is_right_taken = map.get(&(v_right.x))
                .map(|z| z.contains(&v_right.y)).unwrap_or(false);
                
                if !is_right_taken { return fall2(&v_right,y_max, map)}

                return Some(Vec2{x:start.x, y:*y-1})
            },
            _ => Some(Vec2{x:start.x, y:y_max-1})
        },
        _ => Some(Vec2{x:start.x, y:y_max-1})
    }
}
fn main() {
    ...
    let input: HashSet<Vec2> = reader.lines().flat_map(|line| {
        let line = line.unwrap();
        let elements:  Vec<&str> = line.split("->").collect();
        let elements: Vec<Vec2> = elements.iter()
        .map(|x| 
            {let v: Vec<&str> = x.trim().split(",").collect();
            Vec2{x:v.first().unwrap().parse::<i32>().unwrap(), y:v.last().unwrap().parse::<i32>().unwrap()}
        }).collect();
        
         elements.as_slice().windows(2)
        .flat_map(|a| {
            let l = &a[0];
            let r = &a[1];
            (min(l.x,r.x)..max(l.x,r.x)+1)
            .flat_map(|x|{
                (min(l.y,r.y)..max(l.y,r.y)+1)
                .map(move |y|{
        ...
        for i in input {
            ...
            match bmap.get_mut(&i.x) {
                Some(v)=> v.insert(i.y),
                _ => {
                    bmap.insert(i.x, Box::from(BTreeSet::from([i.y]))).is_none()
                }
            };
        }
    ...
    {  // Part 2
        ...
        loop {
            ...
            i += 1;
            if f.as_ref().unwrap() == &start {break}
           
            match f {
                Some(v) => 
                match map.get_mut(&v.x) {
                    Some(y) => {
                        y.insert(v.y);
                        graphical_map.insert(Vec2{x:v.x,y:v.y},Tile::Sand);
                        ()
