    ...
    loop {
        match to_visit.pop() {
            Some((v, s)) => {
                ...
                let q: Vec<(&Vec2, &Tile)> = nexts
                    .iter()
                    .flat_map(|x| (&carte).get(&x).and_then(|t| Some((x, t))))
                    .filter(|(v, t)| {
                        current.can_go_to(t)
                    ...
                    .collect();
                q.into_iter()
                    .for_each(|(v, _)| to_visit.insert(0, (v.to_owned(), s + 1)));
